...
from videox_fun.data.bucket_sampler import (ASPECT_RATIO_632,
...
from videox_fun.utils.lora_utils import create_network, create_kjlora_network, merge_lora
...
def main():
    ...
    if args.kjlora:
        ...
        network.apply_to(transformer3d, True)
    else:
    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            if data_type == 'image':
                ...
                aspect_ratio_sample_size = {key : [x / 632 * args.image_sample_size / random_downsample_ratio for x in ASPECT_RATIO_632[key]] for key in ASPECT_RATIO_632.keys()}
                aspect_ratio_random_crop_sample_size = {key : [x / 632 * args.image_sample_size / random_downsample_ratio for x in ASPECT_RATIO_RANDOM_CROP_632[key]] for key in ASPECT_RATIO_RANDOM_CROP_632.keys()}
                ...
                aspect_ratio_sample_size = {key : [x / 632 * args.video_sample_size / random_downsample_ratio for x in ASPECT_RATIO_632[key]] for key in ASPECT_RATIO_632.keys()}
                aspect_ratio_random_crop_sample_size = {key : [x / 632 * args.video_sample_size / random_downsample_ratio for x in ASPECT_RATIO_RANDOM_CROP_632[key]] for key in ASPECT_RATIO_RANDOM_CROP_632.keys()}
            ...
            for example in examples:
                ...
                if args.train_mode != "normal":
                    mask = get_random_mask(new_examples["pixel_values"][-1].size(), image_start_only=True, image_end=True)
                    ...
                    first_clip_pixel_values = new_examples["pixel_values"][-1][0].permute(1, 2, 0).contiguous()
                    first_clip_pixel_values = (first_clip_pixel_values * 0.5 + 0.5) * 255

                    last_clip_pixel_values = new_examples["pixel_values"][-1][-1].permute(1, 2, 0).contiguous()
                    last_clip_pixel_values = (last_clip_pixel_values * 0.5 + 0.5) * 255

                    new_examples["clip_pixel_values"].append(torch.stack([first_clip_pixel_values, last_clip_pixel_values]))
            ...
            if epoch == first_epoch and step == 0:
                ...
                if args.train_mode != "normal":
                    ...
                    for idx, (clip_pixel_value, pixel_value, text) in enumerate(zip(clip_pixel_values, mask_pixel_values, texts)):
                        pixel_value = pixel_value[None, ...]
                        Image.fromarray(np.uint8(clip_pixel_value[0])).save(f"{args.output_dir}/sanity_check/first_clip_{gif_name[:10] if not text == '' else f'{global_step}-{idx}'}.png")
                        Image.fromarray(np.uint8(clip_pixel_value[1])).save(f"{args.output_dir}/sanity_check/last_clip_{gif_name[:10] if not text == '' else f'{global_step}-{idx}'}.png")
                ...
                with torch.no_grad():
                    ...
                    if args.train_mode != "normal":
                        ...
                        for clip_pixel_value in clip_pixel_values:
                            clip_image = [Image.fromarray(np.uint8(_clip_pixel_value.float().cpu().numpy())) for _clip_pixel_value in clip_pixel_value]
                            clip_image = [TF.to_tensor(_clip_image).sub_(0.5).div_(0.5).to(clip_image_encoder.device, weight_dtype) for _clip_image in clip_image]
                            _clip_contexts = clip_image_encoder([_clip_image[:, None, :, :] for _clip_image in clip_image])

                            for _clip_context in _clip_contexts:
                                if rng is None:
                        ...
                        clip_context = torch.stack(clip_context)
