...
class WanFunInpaintPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        if clip_image is not None:
            if type(clip_image) is not list:
                ...
                first_frame, last_frame = clip_image
                ...
                clip_context = self.clip_image_encoder([first_frame[:, None, :, :], last_frame[:, None, :, :]])
            print(f"---clip_context.shape: {clip_context.shape}---")
            clip_context = (
