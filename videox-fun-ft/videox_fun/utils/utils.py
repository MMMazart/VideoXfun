...
def get_image_to_video_latent(validation_image_start, validation_image_end, video_length, sample_size):
    if validation_image_start is not None and validation_image_end is not None:
        if type(validation_image_start) is str and os.path.isfile(validation_image_start):
            ...
            clip_image_first = Image.open(validation_image_start).convert("RGB").resize([sample_size[1], sample_size[0]])
        else:
        ...
        if type(validation_image_end) is str and os.path.isfile(validation_image_end):
            ...
            clip_image_last = Image.open(validation_image_end).convert("RGB").resize([sample_size[1], sample_size[0]])
        else:
        ...
        if type(image_start) is not list and type(image_end) is not list:
            clip_image = [clip_image_first, clip_image_last]
...
def get_image_to_480_video_latent(validation_image_start, validation_image_end, video_length, max_area=480*832):
    if validation_image_start is not None and validation_image_end is not None:
        if type(validation_image_start) is str and os.path.isfile(validation_image_start):
            image_start = clip_image = Image.open(validation_image_start).convert("RGB")
            
            w, h = image_start.size
            aspect_ratio = h / w
            lat_h = round(np.sqrt(max_area * aspect_ratio) // 8 // 2 * 2)
            lat_w = round(np.sqrt(max_area / aspect_ratio) // 8 // 2 * 2)
            sample_size = (lat_h * 8, lat_w * 8)
    ...
    return  input_video, input_video_mask, clip_image, sample_size
