...
class LoRALinearModule(torch.nn.Module):
    ...
    def __init__(self, lora_name, org_module: torch.nn.Module, multiplier=1.0,
        ...
        self.lora_down = torch.nn.Linear(in_dim, self.lora_dim, bias=False)
        self.lora_up = torch.nn.Linear(self.lora_dim, out_dim, bias=False)
        self.diff_b = torch.nn.Parameter(torch.zeros_like(org_module.bias)) if org_module.bias is not None else None
    ...
    def forward(self, x, *args, **kwargs):
        ...
        if self.diff_b is not None:
            return org_forwarded.to(weight_dtype) + lx.to(weight_dtype) * self.multiplier * scale + self.diff_b.to(weight_dtype)
        else:
            return org_forwarded.to(weight_dtype) + lx.to(weight_dtype) * self.multiplier * scale


class LoRAWanRMSNorm(torch.nn.Module):
    """
    replaces forward method of the original WanRMSNorm, instead of replacing the original Linear module.
    """
    ...
    def forward(self, x, *args, **kwargs):
        ...
        lx = out.to(self.diff.dtype) * self.multiplier * self.scale * self.diff

        return org_forwarded.to(weight_dtype) + lx.to(weight_dtype)
    
    def _norm(self, x):
        return x * torch.rsqrt(x.pow(2).mean(dim=-1, keepdim=True) + self.eps)


class LoRALayerNorm(torch.nn.Module):
    """
    replaces forward method of the original WanRMSNorm, instead of replacing the original Linear module.
    """
    ...
    def forward(self, x, *args, **kwargs):
        ...
        lx = self.multiplier * self.scale * self.diff * x.to(self.diff.dtype)

        if self.diff_b is not None:
            return org_forwarded.to(weight_dtype) + lx.to(weight_dtype) + self.diff_b.to(weight_dtype)
        else:
            return org_forwarded.to(weight_dtype) + lx.to(weight_dtype)
        

class LoRAPatchEmmbed(torch.nn.Module):
    """
    replaces forward method of the original WanRMSNorm, instead of replacing the original Linear module.
    """
    ...
    def forward(self, x, *args, **kwargs):
        ...
        if self.diff_b is not None:
            return org_forwarded.to(weight_dtype) + self.diff_b.to(weight_dtype).view(1, -1, 1, 1, 1)
        else:
            return org_forwarded.to(weight_dtype)


class KiJiaLoRANetwork(torch.nn.Module):
    TRANSFORMER_TARGET_REPLACE_MODULE = ["CogVideoXTransformer3DModel", "WanTransformer3DModel"]
    LORA_PREFIX_TRANSFORMER = "lora_unet"
    def __init__(
        self,
            ...
            for name, module in root_module.named_modules():
                if module.__class__.__name__ in target_replace_modules:
                    for child_name, child_module in module.named_modules():
                        ...
                        is_rmsnorm = child_module.__class__.__name__ == "WanRMSNorm"
                        is_layernorm = child_module.__class__.__name__ == "LayerNorm"
                        is_wanlayernorm = child_module.__class__.__name__ == "WanLayerNorm"

                        if is_linear:
                            lora_name = prefix + "." + name + "." + child_name
                            lora_name = lora_name.replace(".", "_")

                            lora = LoRALinearModule(lora_name, child_module, self.multiplier,
                                self.lora_dim, self.alpha, dropout=dropout)
                            loras.append(lora)
                        ...
                        if is_rmsnorm:
                            ...
                            lora = LoRAWanRMSNorm(lora_name, child_module, self.multiplier,
                                self.lora_dim, self.alpha)
                            loras.append(lora)

                        if is_layernorm or is_wanlayernorm:
                            if child_module.elementwise_affine == True:
                                lora_name = prefix + "." + name + "." + child_name
                                lora_name = lora_name.replace(".", "_")

                                lora = LoRALayerNorm(lora_name, child_module, self.multiplier,
                                    self.lora_dim, self.alpha)
                                loras.append(lora)
                        ...
                        if is_conv3d:
                            ...
                            lora = LoRAPatchEmmbed(lora_name, child_module, self.multiplier,
                                self.lora_dim, self.alpha)
                            loras.append(lora)
            return loras

        self.unet_loras = create_modules(unet, LoRANetwork.TRANSFORMER_TARGET_REPLACE_MODULE)
        print(f"create LoRA for U-Net: {len(self.unet_loras)} modules.")
