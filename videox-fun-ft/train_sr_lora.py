...
from videox_fun.data.dataset_image_video_sr import (ImageVideoDataset,
                                                 ImageVideoSampler)
    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            for example in examples:
                ...
                low_pixel_values = torch.from_numpy(example["low_pixel_values"]).permute(0, 3, 1, 2).contiguous()
                low_pixel_values = low_pixel_values / 255.
                ...
                new_examples["low_pixel_values"].append(transform(low_pixel_values))
            ...
            new_examples["low_pixel_values"] = torch.stack([example[:batch_video_length] for example in new_examples["low_pixel_values"]])
        ...
        for step, batch in enumerate(train_dataloader):
            if epoch == first_epoch and step == 0:
                ...
                for idx, (pixel_value, text) in enumerate(zip(pixel_values, texts)):
                    ...
                    low_pixel_values, texts = batch['low_pixel_values'].cpu(), batch['text']
                    low_pixel_values = rearrange(low_pixel_values, "b f c h w -> b c f h w")
                    for idx, (pixel_value, text) in enumerate(zip(low_pixel_values, texts)):
                        pixel_value = pixel_value[None, ...]
            ...
            with accelerator.accumulate(transformer3d):
                ...
                low_pixel_values = batch["low_pixel_values"].to(weight_dtype)
                ...
                with torch.no_grad():
                    ...
                    if vae_stream_1 is not None:
                        ...
                        latents = _batch_encode_vae(pixel_values)
                        inpaint_latents = _batch_encode_vae(low_pixel_values)
